...
extension HRVChartViewModel {
    ...
    static func mock() -> HRVChartViewModel {
        ...
        let vm = HRVChartViewModel(
            ...
            calculateCurrentScoreUseCase: CalculateCurrentScoreUseCaseImpl(),
            interpretScoreUseCase: InterpretDailyScoreUseCaseImpl(),
            interpretHRVUseCase: InterpretHRVUseCaseImpl(),
            fetchLatestSnapshotUseCase: EmptyLatestHRVSnapshotUseCase(),
            fetchHRBaselineUseCase: EmptyHRBaselineUseCase()
        ...
        let input = HRVInterpretationInput(
            hrvSamples: vm.rmssdPoints,
            hrSamples: vm.heartRateSamples,
...
struct EmptyHRBaselineUseCase: FetchHRBaselineUseCase {
    func execute(days: Int) async throws -> Double? {
        return 0
    }
}

struct EmptyLatestHRVSnapshotUseCase: FetchLatestHRVSnapshotUseCase {
